...
namespace ConsoleAim
{
	...
	public class CommandParser
	{
		...
		private void ExtractApplicationName()
		{
            ...
            string[] strTemp = m_commandLine.Split(' ');
            if (strTemp.Length >= 1)
                m_applicationName = strTemp[0];

            if (strTemp.Length >= 2)
                m_workingString = string.Join(" ", strTemp, 1, strTemp.Length - 1);
            else
                m_workingString = string.Empty;

            //m_applicationName = string.Empty;
            //m_workingString = m_commandLine;
		}
